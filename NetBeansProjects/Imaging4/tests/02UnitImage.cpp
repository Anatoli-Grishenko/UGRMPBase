...
TEST_F(_03_Advanced, Image_ZoomIn) {
   int w, h, v, x, y, whatis, whatmustbe, pos, nimages, histogram[256], element;
    char whatcstringis[6500], whatcstringmustbe[6500];
    string whatstringis, whatstringmustbe;
    w=16;h=16;
    Byte b;
    Image I(w,h),zoomed, dest(2*w,2*h);
    
    I.flatten(Byte(128));
    dest.flatten(Byte(128));
    zoomed = I.zoomIn();
    I.showInWindow("flat");
    zoomed.showInWindow("zoomedinflat");
    zoomed.saveToFile("tests/validation/zomedin.pgm");
    zoomed.depictsHistogram().showInWindow("histo");
    whatis=zoomed.width();
    whatmustbe=dest.width();
    SKIP_ASSERT_EQ_R(whatmustbe,whatis)<<"A zoomed-in image must have twice ts width"<<endl;
    whatis=zoomed.height();
    whatmustbe=dest.height();
    SKIP_ASSERT_EQ_R(whatmustbe,whatis)<<"A zoomed-in image must have twice ts height"<<endl;
    strcpy(whatcstringis,zoomed.inspect().c_str());
    strcpy(whatcstringmustbe,dest.inspect().c_str());
    SKIP_ASSERT_STREQ_R(whatcstringmustbe,whatcstringis) << "A zoomed in flat image continues bein flat"<<endl;
}

TEST_F(_03_Advanced, Image_ZoomOut) {
   int w, h, v, x, y, whatis, whatmustbe, pos, nimages, histogram[256], element;
    char whatcstringis[6500], whatcstringmustbe[6500];
    string whatstringis, whatstringmustbe;
    w=32;h=32;
    Byte b;
    Image I(w,h),zoomed, dest(w/2,h/2);
    
    I.flatten(Byte(128));
    dest.flatten(Byte(128));
    zoomed = I.zoomOut();
    I.showInWindow("flat");
    zoomed.showInWindow("zoomedoutflat");
    zoomed.saveToFile("tests/validation/zomedout.pgm");
    zoomed.depictsHistogram().showInWindow("histo");
    whatis=zoomed.width();
    whatmustbe=dest.width();
    SKIP_ASSERT_EQ_R(whatmustbe,whatis)<<"A zoomed-out image must have half its width"<<endl;
    whatis=zoomed.height();
    whatmustbe=dest.height();
    SKIP_ASSERT_EQ_R(whatmustbe,whatis)<<"A zoomed-out image must have half its height"<<endl;
    strcpy(whatcstringis,zoomed.inspect().c_str());
    strcpy(whatcstringmustbe,dest.inspect().c_str());
    SKIP_ASSERT_STREQ_R(whatcstringmustbe,whatcstringis) << "A zoomed out flat image continues bein flat"<<endl;
}
