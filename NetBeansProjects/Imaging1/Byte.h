...
public:
    ...
    /**
     * @brief It returns the real value of the byte
     * @return The value
     */
    inline int getValue() const { return (int) _data;}
    
    /**
     * @brief It sets a new value
    ...
    /**
    @brief Turns on the bit at plane @a pos   (0 means the rightmost position)
    @param pos 	 the position @p b of the bit within the Byte
     */
    void onBit(int pos);

    /**
    @brief Turns off the bit at plane @a pos   (0 means the rightmost position)
    @param pos 	 the position @p b of the bit within the Byte
     */
    void offBit(int pos);
    ...
    /**
    @brief Turns all the bits on
     */
    void onByte();

    /**
    @brief Turns all the bits off
     */
    void offByte();
    ...
    @param posits An array of positions that contains a bit set to 1
    @param n Number of bits 1 in the byte
     */
     ...
     * a linear combination _data = (_data *(100-percentage)+merge*percentage)/100
